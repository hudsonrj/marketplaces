...
import { createSearchJob } from '../actions'
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                    <thead>
                        <tr style={{ borderBottom: '1px solid rgba(255,255,255,0.05)', background: 'rgba(0,0,0,0.2)' }}>
                            ...
                            <th style={{ padding: '1rem', textAlign: 'left', color: '#94a3b8', fontSize: '0.875rem', fontWeight: '600' }}>PREÇO CUSTO</th>
                    ...
                    <tbody>
                        {products.map((product) => (
                            <tr key={product.id} style={{ borderBottom: '1px solid rgba(255,255,255,0.05)', transition: 'background 0.2s' }}>
                                ...
                                <td style={{ padding: '1rem', color: '#10b981' }}>R$ {Number(product.costPrice).toFixed(2)}</td>
                                ...
                                <td style={{ padding: '1rem', display: 'flex', justifyContent: 'flex-end', gap: '0.5rem' }}>
                                    <form action={async () => {
                                        ...
                                        <button type="submit" className="btn-secondary" style={{ padding: '0.5rem', borderRadius: '6px' }} title="Buscar Agora">
                                            <Search size={16} />
                                        </button>
                                    </form>
